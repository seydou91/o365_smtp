...
1. Switch to the Drupal.org package and run the updates (Drupal 10.3 or later
   is required):

   ```bash
   composer remove seydou91/o365_smtp
   composer require drupal/o365_smtp
   drush updatedb
   drush cr
   ```

2. **Configuration format changed**: `o365_smtp_update_10101` adds
   `helo_hostname` and `max_attachment_size` to `o365_smtp.settings`. Export
   the configuration afterwards (`drush config:export`), otherwise the next
   import removes the new keys.
3. **Redirect URI**: in Microsoft Entra ID, replace
   `https://example.com/phpmailer_oauth2/aad-callback` with
   `https://example.com/admin/config/system/o365_smtp/oauth/callback`.
   Existing tokens keep working; the new URI is needed for the next
   authorization.
4. Check the API permissions: the module needs the **delegated**
   `SMTP.Send` permission (Office 365 Exchange Online) with admin consent, and
   SMTP AUTH enabled on the mailbox. Microsoft Graph `Mail.Send` is not used.
5. Recommended: move the client secret to settings.php
   (`$settings['o365_smtp.client_secret']`), clear it from configuration with
   `drush config:set o365_smtp.settings client_secret ''`, and export the
   configuration again.
6. Custom code attaching files with absolute paths must use `public://`,
   `private://` or `temporary://` URIs, or `filecontent`.
7. Check `/admin/reports/status`, then send a message from the test form.

Breaking changes are detailed at the end of each lot below.
...
- Dead code and draft comments removed. The unused `$auth_url` of the settings
  form and the "Should be random" / "We'll generate the link…" comments went
  away in lot 1, together with the unused mail manager, `$params` and
  `$langcode` of the test form, as part of the dependency injection rewrite.
- The settings form renders the authorization status with the
  `status_messages` theme instead of hard-coded message markup, shows when
  re-authorization is required, and no longer reads State directly.
- MIME boundaries: `md5(uniqid(time()))` no longer exists; since lot 2 they are
  generated by symfony/mime from `random_bytes()`.
- Test form: subject and body fields; on failure it displays the raw last
  response of the SMTP server (`O365Client::getLastSmtpResponse()`).
- README: documents the actual setup (delegated `SMTP.Send` permission, SMTP
  AUTH enabled on the mailbox, admin consent) instead of Microsoft Graph
  `Mail.Send` application permissions, and common Microsoft errors.
